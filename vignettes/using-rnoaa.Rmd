...
The `geom_timeline` displays each observation in a timeline to visualize the dates in which the quakes have played out.  Due to this geom inherit its attributes from the class of `geom_point`, you can use the aesthetics of `size` and `color` to display some other earthquake's attributes such as magnitude and impact of human casualties.

In addition, the package comes with a new ggplot2's theme (i.e., `theme_timeline`) so as to make clearer these geoms.
...
```{r, fig.height = 4}
# Quakes in Mexico from 2000 to 2005.
clean_data %>%
  filter(COUNTRY == "MEXICO",
         !is.na(EQ_PRIMARY),
         YEAR %in% 2000:2005) %>%
  ...
  geom_timeline() +
  # By default, it shows the last three quakes.
